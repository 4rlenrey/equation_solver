...
void Egraph_box::construct_lines()
{
	...
	this->lineY.setPosition(sf::Vector2f(rectangle.getGlobalBounds().width/2, 0)); //width of axix breaks precision
	this->lineY.setSize(sf::Vector2f(3, rectangle.getGlobalBounds().height));

	this->lineX.setFillColor(sf::Color(255, 255, 255));
	this->lineX.setPosition(sf::Vector2f(0, rectangle.getGlobalBounds().height/2)); //need to be fixed
